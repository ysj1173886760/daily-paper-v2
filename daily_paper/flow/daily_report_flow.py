...
def create_daily_report_flow(
    target_date: datetime.date = None, recommendation_count: int = 3
) -> Flow:
    """
    创建日报生成流程
    ...
    analyze_node = AnalyzeAndRecommendPapersNode(
    ...
    *,
    ...
    try:
        ...
        recommendations_count = len(
            shared.get("analysis_and_recommendations", {}).get("recommendations", [])
        )
...
def run_daily_report_with_config(config, target_date: datetime.date = None):
    ...
    async_llm = llm_manager.get_async_llm("analysis")
