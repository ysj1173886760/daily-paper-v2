...
def main():
    ...
    try:
        # 根据任务类型执行不同的流程
        if args.task == "rag":
            logger.info("开始执行RAG论文处理流程")
            result = run_rag_papers()
            logger.info("RAG论文处理流程完成")
        elif args.task == "kg":
            logger.info("开始执行知识图谱论文处理流程")
            result = run_kg_papers()
            logger.info("知识图谱论文处理流程完成")
        else:
            logger.error("未知任务类型，请使用 --task rag 或 --task kg")
            return
        ...
        logger.info(f"日报推送: {'成功' if result.get('daily_report_sent') else '失败/无日报'}")
